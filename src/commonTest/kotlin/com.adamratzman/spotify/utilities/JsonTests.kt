...
class JsonTests : Spek({
    describe("json serialization tests") {
        ...
        it("track serialization") {
            assertTrue(Json.encodeToString(Track.serializer().nullable, api.tracks.getTrack("spotify:track:6kcHg7XL6SKyPNd78daRBL").complete()).isNotEmpty())
        }
        it("album serialization") {
            assertTrue(Json.encodeToString(Album.serializer().nullable, api.albums.getAlbum("spotify:album:6ggQNps98xaXMY0OZWevEH").complete()).isNotEmpty())
        }
