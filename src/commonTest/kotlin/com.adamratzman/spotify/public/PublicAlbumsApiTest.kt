...
class PublicAlbumsApiTest : Spek({
    describe("Public Albums test") {
        ...
        describe("get albums") {
            ...
            it("multiple albums") {
                assertFailsWith<SpotifyException.BadRequestException> { a.getAlbums(market = Market.US).complete() }
                assertFailsWith<SpotifyException.BadRequestException> { a.getAlbums().complete() }
        ...
        describe("get album tracks. market is optional here for relinking support") {
            it("invalid album") {
                assertFailsWith<SpotifyException.BadRequestException> { a.getAlbumTracks("no").complete() }
            }
            it("valid album") {
