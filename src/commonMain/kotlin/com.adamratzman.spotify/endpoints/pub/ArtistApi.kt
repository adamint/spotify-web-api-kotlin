...
public class ArtistApi(api: GenericSpotifyApi) : SpotifyEndpoint(api) {
     ...
     * @return [Artist] if valid artist id is provided, otherwise null
     */
    public fun getArtistRestAction(artist: String): SpotifyRestAction<Artist?> = SpotifyRestAction { getArtist(artist) }
     ...
     * @return List of [Artist] objects or null if the artist could not be found, in the order requested
     */
    public fun getArtistsRestAction(vararg artists: String): SpotifyRestAction<List<Artist?>> = SpotifyRestAction {
        getArtists(*artists)
    }
     ...
     * @return [PagingObject] of [SimpleAlbum] objects
     */
    public fun getArtistAlbumsRestAction(
        artist: String,
        limit: Int? = api.spotifyApiOptions.defaultLimit,
        offset: Int? = null,
        market: Market? = null,
        vararg include: AlbumInclusionStrategy
    ): SpotifyRestAction<PagingObject<SimpleAlbum>> = SpotifyRestAction { getArtistAlbums(artist, limit, offset, market, *include) }
     ...
     * @return List of the top [Track]s of an artist in the given market
     */
    public fun getArtistTopTracksRestAction(artist: String, market: Market = Market.US): SpotifyRestAction<List<Track>> = SpotifyRestAction {
        getArtistTopTracks(artist, market)
    }
     ...
     * @return List of *never-null*, but possibly empty [Artist]s representing similar artists
     */
    public suspend fun getRelatedArtistsRestAction(artist: String): SpotifyRestAction<List<Artist>> = SpotifyRestAction {
