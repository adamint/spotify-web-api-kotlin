...
public class AlbumApi(api: GenericSpotifyApi) : SpotifyEndpoint(api) {
     ...
     * @return Full [Album] object if the provided id is found, otherwise null
     */
    public fun getAlbumRestAction(album: String, market: Market? = null): SpotifyRestAction<Album?> = SpotifyRestAction {
        getAlbum(album, market)
    }
     ...
     * @return List of [Album] objects or null if the album could not be found, in the order requested
     */
    public fun getAlbumsRestAction(vararg albums: String, market: Market? = null): SpotifyRestAction<List<Album?>> = SpotifyRestAction {
        getAlbums(*albums, market = market)
    }
     ...
     * @return [PagingObject] of [SimpleTrack] objects
     */
    public fun getAlbumTracksRestAction(
        album: String,
        limit: Int? = api.spotifyApiOptions.defaultLimit,
        offset: Int? = null,
        market: Market? = null
    ): SpotifyRestAction<PagingObject<SimpleTrack>> = SpotifyRestAction { getAlbumTracks(album, limit, offset, market) }
}
